...
type Request struct {
	...
	// Hostname that is used only in Host header
	requestHostname []byte
...
func (req *Request) SetIP(ipAddr string) {
	req.ipAddr = ipAddr
}

func (req *Request) SetDial(dial DialFunc) {
	req.dial = dial
}
...
// SetRequestRaw sets raw request, but without copying it.
//
// From this point onward the rawRequest argument must not be changed.
func (req *Request) SetRequestRaw(rawRequest []byte) {
	req.ResetRequestRaw()
	req.ResetBody()
	req.requestRaw = rawRequest
}

// SetRequestHostname sets hostname that is used only in Host header
//
// From this point onward the requestHostname argument must not be changed.
func (req *Request) SetRequestHostname(requestHostname []byte) {
	req.requestHostname = requestHostname
}
...
// ResetBody resets request body.
func (req *Request) ResetRequestRaw() {
	req.requestRaw = nil
}
...
func (resp *Response) Read(r *bufio.Reader) error {
	return resp.ReadLimitBody(r, 0, 0)
}
...
//
// io.EOF is returned if r is closed before reading the first header byte.
func (resp *Response) ReadLimitBody(r *bufio.Reader, maxBodySize int, readFirstNBodyBytes int) error {
	resp.resetSkipHeader()
	...
	if !resp.mustSkipBody() {
		return resp.ReadBody(r, maxBodySize, readFirstNBodyBytes)
	}
	return nil
}
...
// If maxBodySize > 0 and the body size exceeds maxBodySize,
// then ErrBodyTooLarge is returned.
func (resp *Response) ReadBody(r *bufio.Reader, maxBodySize int, readFirstNBodyBytes int) (err error) {
	bodyBuf := resp.bodyBuffer()
	bodyBuf.Reset()
	// fmt.Printf("Status %v %v %v\n", resp.Header.StatusCode(), resp.Header.ContentLength(), maxBodySize)

	// sometimes ContentLength will be set to -1 and if the response is a redirect we do not want to read response body
	if readFirstNBodyBytes > 0 && (readFirstNBodyBytes < resp.Header.ContentLength() || resp.Header.ContentLength() == -1) {
		bodyBuf.B, err = readBodyNBytes(r, resp.Header.ContentLength(), maxBodySize, bodyBuf.B, readFirstNBodyBytes)
	} else {
...
// ReadBytesFromBody reads N bytes from response body from the given r
func (resp *Response) ReadBytesFromBody(r *bufio.Reader, n int) (err error) {
	bodyBuf := resp.bodyBuffer()
	bodyBuf.Reset()

	if n == 0 {
		bodyBuf.B, err = readBody(r, resp.Header.ContentLength(), 0, bodyBuf.B)
	} else {
		bodyBuf.B, err = readBody(r, n, 0, bodyBuf.B)
	}
...
func (req *Request) Write(w *bufio.Writer) error {
	// if the request is raw, we skip this logic
	if (len(req.Header.Host()) == 0 || req.parsedURI) && len(req.requestRaw) == 0 {
		uri := req.URI()
		host := uri.Host()
		if len(host) == 0 {
...
func (resp *Response) brotliBody(level int) error {
	...
	if resp.bodyStream != nil {
		...
		// For https://rawfasthttp/issues/176 .
...
func readBodyNBytes(r *bufio.Reader, contentLength int, maxBodySize int, dst []byte, nBytes int) ([]byte, error) {
	dst = dst[:0]
	if contentLength >= 0 {
		return appendBodyFixedSize(r, dst, nBytes)
	}
	if contentLength == -1 {
		return readBodyChunkedNBytes(r, maxBodySize, dst, nBytes)
	}
	return readBodyIdentity(r, maxBodySize, dst)
}
...
// TODO dodac testy
func readBodyChunkedNBytes(r *bufio.Reader, maxBodySize int, dst []byte, nBytes int) ([]byte, error) {
	if len(dst) > 0 {
		panic("BUG: expected zero-length buffer")
	}
	...
	for {
		...
		if nBytes > 0 && len(dst)+chunkSize > nBytes {
			dst, err = appendBodyFixedSize(r, dst, nBytes-len(dst))
		} else {
			dst, err = appendBodyFixedSize(r, dst, chunkSize+strCRLFLen)
		}

		if nBytes > 0 && len(dst) >= nBytes {
			return dst, nil
		}
