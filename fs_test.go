...
func TestServeFileDirectoryRedirect(t *testing.T) {
	...
	ctx.Request.Reset()
	ctx.Response.Reset()
	ServeFile(&ctx, "rawfasthttputil")
	if ctx.Response.StatusCode() != StatusFound {
		t.Fatalf("Unexpected status code %d for directory '/rawfasthttputil' without trailing slash. Expecting %d.", ctx.Response.StatusCode(), StatusFound)
	}
	...
	if ctx.Response.StatusCode() != StatusOK {
		t.Fatalf("Unexpected status code %d for directory '/rawfasthttputil/' with trailing slash. Expecting %d.", ctx.Response.StatusCode(), StatusOK)
	}
