...
func DoRedirects(req *Request, resp *Response, maxRedirectsCount int) error {
	_, _, err := doRequestFollowRedirects(req, resp, req.URI().String(), maxRedirectsCount, &defaultClient, func(_ []byte) bool {
		return true
	})
	return err
}

func DoControlledRedirects(req *Request, resp *Response, maxRedirectsCount int, shouldFollow func([]byte) bool) error {
	_, _, err := doRequestFollowRedirects(req, resp, req.URI().String(), maxRedirectsCount, &defaultClient, shouldFollow)
	return err
}
...
type Client struct {
	...
	// header names. See https://rawfasthttp/issues/57
	// for details.
	//
...
func (c *Client) DoRedirects(req *Request, resp *Response, maxRedirectsCount int) error {
	_, _, err := doRequestFollowRedirects(req, resp, req.URI().String(), maxRedirectsCount, c, func(location []byte) bool {
		return true
	})
	return err
}

func (c *Client) DoControlledRedirects(req *Request, resp *Response, maxRedirectsCount int, shouldFollow func([]byte) bool) error {
	_, _, err := doRequestFollowRedirects(req, resp, req.URI().String(), maxRedirectsCount, c, shouldFollow)
	return err
}
...
func (c *Client) Do(req *Request, resp *Response) error {
	...
	if hc == nil {
		hc = &HostClient{
			...
			ReadFirstNBodyBytes:           req.ReadFirstNBodyBytes,
...
type HostClient struct {
	...
	// Sometimes we will want to read only first N bytes of the body (when we know that te response is big)
	ReadFirstNBodyBytes int
...
func doRequestFollowRedirectsBuffer(req *Request, dst []byte, url string, c clientDoer) (statusCode int, body []byte, err error) {
	...
	statusCode, _, err = doRequestFollowRedirects(req, resp, url, defaultMaxRedirectsCount, c, func(location []byte) bool {
		return true
	})
...
func doRequestFollowRedirects(req *Request, resp *Response, url string, maxRedirectsCount int, c clientDoer, shouldFollow func([]byte) bool) (statusCode int, body []byte, err error) {
	redirectsCount := 0

	redirect := false

	for {
		if redirect && len(req.requestRaw) > 0 {
			redirectLocationURI := URI{}
			redirectLocationURI.Parse(nil, []byte(url))

			req.requestRaw, err = replacePathAndHostInRawRequest(
				req.requestRaw,
				redirectLocationURI.path,
				req.requestHostname,
				redirectLocationURI.Host(),
			)
			...
			// we set hostname from redirect
			req.SetRequestHostname(redirectLocationURI.Host())
		}
...
func replacePathAndHostInRawRequest(raw []byte, newPath []byte, oldHost []byte, newHost []byte) ([]byte, error) {
	rawString := string(raw)

	// we will replace old host with new host
	rawString = strings.Replace(rawString, string(oldHost), string(newHost), 1)

	lines := strings.SplitN(rawString, "\r\n", 2)

	if len(lines) > 0 {
		parts := strings.Split(lines[0], " ")

		if len(parts) > 2 {
			updatedLineWithPath := fmt.Sprintf("%s %s %s\r\n", parts[0], newPath, parts[2])

			return []byte(updatedLineWithPath + lines[1]), nil
		}
	}

	return []byte(rawString), fmt.Errorf("cannot replace path in raw request: %q", raw)
}

// func IsRelativeRedirect(currentURI *URI, location string) bool {
// 	locationURI := URI{}
// 	locationURI.Parse(nil, []byte(location))
...
// 	sameHost := bytes.Equal([]byte(currentURI.Host()), locationURI.Host()) &&
// 		bytes.Equal([]byte(currentURI.Scheme()), locationURI.Scheme())
...
func (c *HostClient) doNonNilReqResp(req *Request, resp *Response) (bool, error) {
	...
	if len(userAgentOld) == 0 && len(req.requestRaw) == 0 {
	...
	if err = resp.ReadLimitBody(br, c.MaxResponseBodySize, c.ReadFirstNBodyBytes); err != nil {
		c.releaseReader(br)
		c.closeConn(cc)
