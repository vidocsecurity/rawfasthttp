...
func TestNewFastHTTPHandler(t *testing.T) {
	...
	expectedMethod := rawfasthttp.MethodPost
	...
	var ctx rawfasthttp.RequestCtx
	...
	resp := &ctx.Response
	if resp.StatusCode() != rawfasthttp.StatusBadRequest {
		t.Fatalf("unexpected statusCode: %d. Expecting %d", resp.StatusCode(), rawfasthttp.StatusBadRequest)
	}
...
func setContextValueMiddleware(next rawfasthttp.RequestHandler, key string, value interface{}) rawfasthttp.RequestHandler {
