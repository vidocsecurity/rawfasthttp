...
func testServerRequests(t *testing.T, ln *rawfasthttputil.InmemoryListener) {
	conn, err := ln.Dial()
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
...
func TestMultipleServe(t *testing.T) {
	...
	ln1 := rawfasthttputil.NewInmemoryListener()
	ln2 := rawfasthttputil.NewInmemoryListener()
...
func TestStreamRequestBody(t *testing.T) {
	...
	pipe := rawfasthttputil.NewPipeConns()
	...
	select {
	case err := <-ch:
		if err == nil || err.Error() != "connection closed" { // rawfasthttputil.errConnectionClosed is private so do a string match.
			t.Fatalf("Unexpected error from serveConn: %s", err)
		}
	case <-time.After(500 * time.Millisecond):
