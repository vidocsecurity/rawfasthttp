...
func benchmark(b *testing.B, h rawfasthttp.RequestHandler, isTLS bool) {
	bc := &benchConfig{
		IsTLS: isTLS,
	}
	benchmarkExt(b, h, bc)
...
func benchmarkExt(b *testing.B, h rawfasthttp.RequestHandler, bc *benchConfig) {
	...
	ln := rawfasthttputil.NewInmemoryListener()
	...
	go func() {
		...
		if err := rawfasthttp.Serve(serverLn, h); err != nil {
...
func streamingHandler(ctx *rawfasthttp.RequestCtx) {
	ctx.WriteString("foobar") //nolint:errcheck
}

func handshakeHandler(ctx *rawfasthttp.RequestCtx) {
...
func runRequests(b *testing.B, pb *testing.PB, c *rawfasthttp.HostClient) {
	...
	for pb.Next() {
		...
		if resp.StatusCode() != rawfasthttp.StatusOK {
			b.Fatalf("unexpected status code: %d. Expecting %d", resp.StatusCode(), rawfasthttp.StatusOK)
		}
	}
}
