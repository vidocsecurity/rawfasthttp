...
// FasthttpHTTPDialerTimeout returns a rawfasthttp.DialFunc that dials using
...
func FasthttpHTTPDialerTimeout(proxy string, timeout time.Duration) rawfasthttp.DialFunc {
	...
	return func(addr string) (net.Conn, error) {
		...
		var err error
		if timeout == 0 {
			conn, err = rawfasthttp.Dial(proxy)
		} else {
			conn, err = rawfasthttp.DialTimeout(proxy, timeout)
		}
		if err != nil {
			return nil, err
		}
