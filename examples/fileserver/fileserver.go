...
func main() {
	...
	if *vhost {
		fs.PathRewrite = rawfasthttp.NewVHostPathRewriter(0)
	}
	...
	if len(*addr) > 0 {
		...
		go func() {
			if err := rawfasthttp.ListenAndServe(*addr, requestHandler); err != nil {
	...
	if len(*addrTLS) > 0 {
		...
		go func() {
			if err := rawfasthttp.ListenAndServeTLS(*addrTLS, *certFile, *keyFile, requestHandler); err != nil {
...
func updateFSCounters(ctx *rawfasthttp.RequestCtx) {
	...
	case rawfasthttp.StatusOK:
		fsOKResponses.Add(1)
		fsResponseBodyBytes.Add(int64(resp.Header.ContentLength()))
	case rawfasthttp.StatusNotModified:
		fsNotModifiedResponses.Add(1)
	case rawfasthttp.StatusNotFound:
